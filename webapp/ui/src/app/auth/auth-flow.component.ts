...
const PASSWORD_RULE_NAMES: Record<string, string> = {
  length: '8 characters',
  'lower-case': 'a lowercase letter',
  'upper-case': 'an uppercase letter',
  digits: 'a digit',
  'special-chars': 'a symbol'
};
...
export class AuthFlowComponent implements OnInit {
  ...
  missingPasswordRules(errors: ValidationErrors | null): string {
    // The order of the rules rather than the error keys: length first, as it is read.
    return Object.keys(PASSWORD_RULE_NAMES)
      .filter((key) => errors?.[key])
      .map((key) => PASSWORD_RULE_NAMES[key])
      .join(', ');
  }
